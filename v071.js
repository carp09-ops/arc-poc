// Arc 0.7.1 — Passive Morning + Device Evidence
(function(){
  ...
  const sleepHours=()=>{
    ...
    return samples.filter(x=>x.sample_type==='sleep_analysis'&&String(x.value_text||'').startsWith('asleep')&&new Date(x.end_at||x.start_at).getTime()>cutoff)
      .reduce((a,x)=>a+Math.max(0,(new Date(x.end_at||x.start_at)-new Date(x.start_at))/3600000),0);
  };
  ...
  async function load(force=false){
    ...
    try{
      ...
      samples=hs||[];appleSource=ds||null;lastLoaded=Date.now();renderPassive();decorateModal();decorateProvenance();
  ...
  function cardMarkup(){
    const s=summary();
    if(!s.has){
      return `<section class="arc-passive-morning waiting">
        <div class="arc-passive-lead"><div class="arc-passive-orbit"><img src="assets/arc-monogram-transparent.svg" alt=""></div><div><span class="eyebrow">PASSIVE MORNING</span><h2>The next version of Today is mostly automatic.</h2><p>When the native iOS pilot connects Apple Health, Arc can wake up with the objective parts of your day already assembled.</p></div></div>
        <div class="arc-passive-grid">${metric('SLEEP','—','Apple Health',false)}${metric('RESTING HR','—','Apple Health',false)}${metric('ACTIVITY','—','Apple Health',false)}${metric('WEIGHT','—','Compatible scale',false)}</div>
        <div class="arc-passive-foot"><span><i></i>Waiting for first HealthKit sync</span><button data-connected-data>View Connected Data →</button></div>
      </section>`;
    }
    const observed=[s.sleep>0,s.rhr,Number.isFinite(s.steps)&&s.steps>0,s.weight].filter(Boolean).length;
    return `<section class="arc-passive-morning live">
      <div class="arc-passive-lead"><div class="arc-passive-orbit"><img src="assets/arc-monogram-transparent.svg" alt=""></div><div><span class="eyebrow">PASSIVE MORNING · ${observed} STREAM${observed===1?'':'S'} OBSERVED</span><h2>${s.sleep>0||s.rhr?'Arc already has your night.':'Arc already has your devices.'}</h2><p>Objective context arrived without another form. Add only the part a device cannot know.</p></div></div>
      <div class="arc-passive-grid">
        ${metric('SLEEP',s.sleep>0?`${fmt1(s.sleep)}h`:'—',s.sleep>0?'Observed overnight':'No sleep sample')}
        ${metric('RESTING HR',s.rhr?`${Math.round(+s.rhr.value_numeric)} bpm`:'—',s.rhr?sourceName(s.rhr):'No reading')}
        ${metric('STEPS',s.steps?s.steps.toLocaleString():'—',s.steps?'Observed today':'No steps yet')}
        ${metric('WEIGHT',s.weight?`${fmt1(s.weight.value_numeric)} lb`:'—',s.weight?sourceName(s.weight):'No device weight')}
      </div>
      <div class="arc-passive-secondary"><span>${s.energy?`Active energy · ${s.energy} kcal`:''}</span><span>${s.workout?`Latest workout · ${Math.round(+s.workout.value_numeric||0)} min`:''}</span></div>
      <div class="arc-passive-foot"><span class="live-status"><i></i>${appleSource?.last_sync_at?`Synced ${new Date(appleSource.last_sync_at).toLocaleString(undefined,{month:'short',day:'numeric',hour:'numeric',minute:'2-digit'})}`:'Apple Health evidence received'}</span><button data-context-jump>How do you feel today? →</button></div>
    </section>`;
  }
  ...
  function renderPassive(){
    ...
    let el=stage.querySelector('.arc-passive-morning');
    const html=cardMarkup();
    if(el){el.outerHTML=html;return}
    const anchor=stage.querySelector('.cinema-hero');
    if(anchor)anchor.insertAdjacentHTML('afterend',html);else stage.insertAdjacentHTML('afterbegin',html);
  }

  function decorateProvenance(){
    const s=summary();
    if(s.weight){
      const head=document.querySelector('.v052-weight-card .v052-card-head');
      const chip=head?.querySelector('.arc-source-chip');
      if(chip){chip.className='arc-source-chip device';chip.innerHTML='<i></i>APPLE HEALTH';}
    }
  }
  ...
  document.addEventListener('click',e=>{
    if(e.target.closest('[data-context-jump]')){
      document.querySelector('.v052-context-card')?.scrollIntoView({behavior:'smooth',block:'center'});
    }
    if(e.target.closest('[data-connected-data]'))setTimeout(()=>{load(true);decorateModal()},80);
  });
  document.addEventListener('visibilitychange',()=>{if(!document.hidden)load(true)});

  const mo=new MutationObserver(()=>{renderPassive();decorateProvenance();if(document.querySelector('.arc-connect-modal'))decorateModal()});
  mo.observe(document.body,{childList:true,subtree:true});

  async function boot(){
    for(let i=0;i<40&&!user();i++)await new Promise(r=>setTimeout(r,200));
    renderPassive();
